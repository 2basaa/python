...
def restrigin(x, n=None, A=10.):
    if n is None:
        n = x.shape[0]
    return A*n + np.sum(x**2 - A*np.cos(2.0*np.pi*x))

def restrigin_df_analytic(x, A=10.0):
    return 2.0*x + A*np.sin(2.0*np.pi*x)*2.0*np.pi

def restrigin_h_analytic(x, A=10.0):
    N = len(x)
    H = np.zeros((N, N))
    for i in range(N):
